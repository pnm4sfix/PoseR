...
class DropCatcher(QLabel):
    def __init__(self, parent=None, on_files_dropped=None):
        ...
        self.setStyleSheet("background-color: #444; color: white; padding: 20px;")
    ...
    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            ...
            print(f"📦 DropCatcher got files: {paths}")
...
class PoserWidget(QWidget):
    ...
    def __init__(self, napari_viewer):
        ...
        # Your custom QWidget (e.g. DropCatcher)
        self.drop_catcher = DropCatcher(on_files_dropped=self.handle_dragged_files)
        layout.addWidget(self.drop_catcher)  # ✅ Add QLabel-based widget directly

        # Your magicgui container for all GUI widgets
        self.container = Container()
        layout.addWidget(self.container.native)  # ✅ Embed using .native

        #✅ Define widgets BEFORE appending
        self.label = Label(value="Drag a file onto this window!")
        ...
        self.container.append(self.vid_picker)


        self.decoder_dir_picker = FileEdit(
            label="Select Decoder Directory",
            value="./",
            tooltip="Select folder containing decoder files",
            mode="d"  # 'd' for directory mode
        )

        
        self.decoder_dir_picker.changed.connect(self.decoder_dir_changed)
        
        self.config_file_picker = FileEdit(
            label="Select Config File",
            value="./",
            tooltip="Select decoder_config.yml",
            mode="d"
        )
        self.config_file_picker.changed.connect(self.config_file_changed)
        ...
        # Inside your __init__ method of PoserWidget
        self.model_label = Label(value="Select Model:")
        self.model_selector = ComboBox(
            choices=["ST-GCN-18", "ST-GCN-LSTM", "ST-GCN-18-3BLOCK"]
        )

        # Use extend to add widgets to the Container
        self.container.extend([self.model_label, self.model_selector])

        # Connect signals
        self.model_selector.changed.connect(self.on_model_change)
        ...
        self.container.extend([self.behavioural_extract_method])
        ...
        self.push_button = PushButton(label="Save Labels")
        self.push_button.changed.connect(self.save_classification_data)
        ...
        self.retrain_button = PushButton(label="Retrain Model")
        self.retrain_button.clicked.connect(self.retrain)
        self.container.extend([self.retrain_button])
    ...
    def config_file_changed(self, event):
        try:
            try:
                self.config_file = event.value.value
            except:
                self.config_file = event.value
            ...
            # Try accessing required fields to validate structure
            ...
            show_info(f"✅ Successfully loaded config: {os.path.basename(self.config_file)}")

            self.populate_chkpt_dropdown()

        except Exception as e:
            
            show_info(f"Failed to load config. Please check the file.")
    ...
    def handle_dragged_files(self, paths):
        ...
        print("📥 handle_dragged_files called")
        ...
        for path in paths:
            ...
            if path.lower().endswith((".avi", ".mp4", ".mov", ".mkv")):
                print(f"🎥 Setting video: {path}")
                self.video_file = path
                self.vid_picker.value = path

            elif path.lower().endswith(".h5"):
                print(f"📊 Setting HDF5: {path}")
                self.h5_picker.value = path

            elif path.lower().endswith(".txt"):
                print(f"📝 Setting TXT: {path}")
                self.label_txt_picker.value = path

            elif path.lower().endswith((".yml", ".yaml")):
                print(f"⚙️ Setting config: {path}")
                self.config_file_picker.value = path
    ...
    def dropEvent(self, event):
        ...
        if event.mimeData().hasUrls():
            paths = [url.toLocalFile() for url in event.mimeData().urls()]
            print(f"📥 Drag-dropped files: {paths}")
    ...
    def handle_dragged_files(self, paths):
        ...
        for path in paths:
            print(f"Processing dropped file: {path}")
            if path.lower().endswith((".avi", ".mp4", ".mov", ".mkv")):
                self.vid_picker_changed(type('event', (object,), {'value': path}))
            elif path.lower().endswith(".h5"):
                self.h5_picker_changed(type('event', (object,), {'value': path}))
            elif path.lower().endswith(".txt"):
                self.convert_txt_todict(type('event', (object,), {'value': path}))
            elif path.lower().endswith((".yml", ".yaml")):
                self.config_file_changed(type('event', (object,), {'value': path}))
    ...
    def decoder_dir_changed(self, value):
        ...
        # commented out as the config file is now loaded only through picker. 
        """
    ...
    def get_points(self):
        if not hasattr(self, "coords_data") or not self.coords_data:
            show_info("No coordinate data found — run read_coords() first.")
            return

        ind = "individual1"
        if ind not in self.coords_data:
            show_info(f"⚠️ Individual '{ind}' not found in coords_data.")
            return

        x = self.coords_data[ind]["x"]
        y = self.coords_data[ind]["y"]

        if x is None or y is None:
            show_info("x or y data missing.")
            return

        T, V = x.shape  # T = frames, V = bodyparts
        self.points = np.zeros((T * V, 2))
        self.points[:, 0] = x.flatten()
        self.points[:, 1] = y.flatten()
        ...
        print(f"Points layer added with shape {self.points.shape}")
    ...
    def movement_labels(self):
        ...
        if self.points is None:
            print("Warning: Points are not loaded. Cannot label movement.")
            return
    ...
    def h5_picker_changed(self, event):
        ...
        # verify DLC data is correctly loaded! 
        print(f"Number of coordinates loaded: {len(self.coords_data)}")
        print(f"Keys: {list(self.coords_data.keys())}")
        print(f"First key contents: {self.coords_data.get(1)}")

        
        
        # gui forces individual parameter to 1 automatically - 0 causes logic issues. 
        self.ind_spinbox.max = max(self.coords_data.keys())
    ...
    def vid_picker_changed(self, event):
        ...
        print("🎬 vid_picker_changed triggered")
        print(f"🧪 event.value: {event.value} ({type(event.value)})")
        
        try:
            self.video_file = event.value.value  # magicgui FileEdit
        except:
            self.video_file = str(event.value)

        if not os.path.isfile(self.video_file):
            print(f"❌ Not a valid file: {self.video_file}")
            return
            
       
        
        print(f"Video file changed to {self.video_file}")
        
        # After setting self.video_file
        video_dir = os.path.dirname(self.video_file)

        # Look for files
        potential_config = os.path.join(video_dir, "decoder_config.yml")
        potential_dlc_h5 = None
        potential_label_h5 = None
        potential_label_txt = None

        for file in os.listdir(video_dir):
            if file.endswith(".h5") and "labeled" not in file.lower():
                potential_dlc_h5 = os.path.join(video_dir, file)
            if file.endswith(".h5") and "labeled" in file.lower():
                potential_label_h5 = os.path.join(video_dir, file)
            if file.endswith(".txt"):
                potential_label_txt = os.path.join(video_dir, file)

        # Now auto-load by setting .value (ONLY set .value)
        if os.path.exists(potential_config):
            self.config_file_picker.value = potential_config
            show_info(f"Loaded config file: {os.path.basename(potential_config)}")

        if potential_dlc_h5:
            self.h5_picker.value = potential_dlc_h5
            show_info(f"Loaded DLC h5 file: {os.path.basename(potential_dlc_h5)}")

        if potential_label_h5:
            self.label_h5_picker.value = potential_label_h5
            show_info(f"Loaded labeled h5 file: {os.path.basename(potential_label_h5)}")

        if potential_label_txt:
            self.label_txt_picker.value = potential_label_txt
            show_info(f"Loaded labeled txt file: {os.path.basename(potential_label_txt)}")
        ...
        if not hasattr(self, "x") or not hasattr(self, "y"):
            ...
            default_h5 = base + "test_DLC_file.h5"  # Safer than replacing .avi only

            if os.path.exists(default_h5):
                print(f"Found matching DLC file: {default_h5}")
                self.h5_picker_changed(type('event', (object,), {'value': default_h5}))
            else:
        ...
        if self.video_file.lower().endswith((".avi", ".mp4", ".mov", ".mkv")):
            ...
            if self.im is not None:
                ...
                # Safely force individual parameter to 1
                if hasattr(self, "coords_data") and self.coords_data:
                    self.ind_spinbox.max = max(self.coords_data.keys())
                    self.ind_spinbox.value = 1
                    self.ind = 1
                else:
                    print("Warning: coords_data missing when setting individual.")
    ...
    def automatic_process_video(self):
        ...
        print(f"📂 VIDEO FILE VALUE: {self.video_file} (type: {type(self.video_file)})")
        
        print("🔄 Starting flexible video processing...")

        if not self.video_file:
            show_info("❌ No video file loaded.")
            return

        video_path = str(self.video_file)
        base_dir = os.path.dirname(video_path)
        print(f"Video directory: {base_dir}")

        # Search all files in the directory
        all_files = os.listdir(base_dir)

        # Use keyword-based matching instead of strict filenames
        dlc_h5_file = next((os.path.join(base_dir, f) for f in all_files if f.endswith(".h5") and "DLC" in f.upper()), None)
        label_h5_file = next((os.path.join(base_dir, f) for f in all_files if f.endswith(".h5") and "classification" in f.lower()), None)
        txt_file = next((os.path.join(base_dir, f) for f in all_files if f.endswith(".txt")), None)
        config_file = next((os.path.join(base_dir, f) for f in all_files if f.endswith(".yml") and "config" in f.lower()), None)

        # Log the found files
        print(f"🔍 Found DLC H5: {dlc_h5_file}")
        print(f"🔍 Found Classification H5: {label_h5_file}")
        print(f"🔍 Found Config: {config_file}")
        print(f"🔍 Found TXT: {txt_file}")

        # Auto-set FileEdit values and trigger processing methods
        if config_file and os.path.isfile(config_file):
            self.config_file_changed(type('event', (object,), {'value': config_file}))
            show_info(f"🟢 Loaded config: {os.path.basename(config_file)}")
        else:
            show_info("⚠️ Config file not found.")

        if dlc_h5_file and os.path.isfile(dlc_h5_file):
            self.h5_picker_changed(type('event', (object,), {'value': dlc_h5_file}))
            show_info(f"🟢 Loaded DLC h5: {os.path.basename(dlc_h5_file)}")
        else:
            show_info("⚠️ DLC h5 file not found.")

        if label_h5_file and os.path.isfile(label_h5_file):
            self.convert_h5_todict(type('event', (object,), {'value': label_h5_file}))
            show_info(f"🟢 Loaded labelled h5: {os.path.basename(label_h5_file)}")
        else:
            show_info("⚠️ Labelled h5 file not found.")

        if txt_file and os.path.isfile(txt_file):
            self.convert_txt_todict(type('event', (object,), {'value': txt_file}))
            show_info(f"🟢 Loaded txt: {os.path.basename(txt_file)}")

        # Ensure coords are loaded
        self.get_points()
        ...
        show_info("✅ Auto-processing complete.")
    ...
    def convert_h5_todict(self, event):
        ...
        # NEW: Check that it's a valid file
        if not os.path.isfile(self.labeled_h5):
            show_info(f"Selected file is not a valid HDF5 file: {self.labeled_h5}")
            return

        try:
            self.labeled_h5_file = tb.open_file(self.labeled_h5, mode="a")
        except Exception as e:
            show_info(f"Failed to open HDF5 file: {e}")
            return
    ...
    def classification_data_to_ethogram(self):
        ...
        if not hasattr(self, 'im') or self.im is None: 
            show_info("No DLC image data ('im') loaded and therefore cannot compute the ethogram")
            return None 
    ...
    def extract_behaviours(self):


        h5_path = self.h5_picker.value
        
        if not h5_path or os.path.isdir(h5_path) or not os.path.isfile(h5_path):
            show_info(f"Invalid h5 file path: {h5_path}")
            return
        ...
        if not os.path.exists(h5_path):
            show_info("H5 path does not exist.")
            return

        with pd.HDFStore(h5_path, 'r') as h5file:
            keys = h5file.keys()
            print("Available keys in H5 file:", keys)
            df = h5file.get(keys[0])
            print("DataFrame head:", df.head())

       
        #check if points are loaded correctly
        if self.points is None:
            print("points not loaded. Trying to load points now. ")
        try:
            self.get_points()
            if self.points is None:
                raise ValueError("Points could not be loaded")
            print("Points successfully loaded")
        except Exception as e:
            show_info("Failed to load points")
            return
        ...
        if self.extract_method == "orth":
            ...
            print("Unknown extraction method...")
    ...
    def read_coords(self, h5_file):
        ...
        print(f"Reading DLC coordinates from: {h5_file}")

        try:
            df = pd.read_hdf(h5_file, key="/df_with_missing")
        except Exception as e:
            show_info(f"❌ Failed to read DLC .h5 file: {e}")
            return

        print("DLC data loaded successfully")
        print("Column levels:", df.columns.names)
        print("First few columns:", df.columns[:6])

        # 🛠 Drop 'scorer' level if present
        if 'scorer' in df.columns.names:
            df.columns = df.columns.droplevel('scorer')
            print("Dropped 'scorer' level from MultiIndex.")

        # Now df.columns should be MultiIndex with ['bodyparts', 'coords']
        if df.columns.nlevels != 2 or df.columns.names != ['bodyparts', 'coords']:
            show_info("❌ Unexpected H5 format. Expected MultiIndex with ['bodyparts', 'coords']")
            return
        ...
        try:
            ...
            if "likelihood" in df.columns.get_level_values("coords"):
                ci = df.xs("likelihood", level="coords", axis=1)
                print("✅ Found likelihoods")
            else:
                ci = pd.DataFrame(1.0, index=x.index, columns=x.columns)
                print("⚠️ No likelihoods found — using 1.0 for confidence.")
        except Exception as e:
            show_info(f"❌ Failed to extract coordinate data: {e}")
            return

        # Store as NumPy arrays
        self.coords_data["individual1"] = {
            "x": x.values,
            "y": y.values,
            "ci": ci.values,
        }
        self.x["individual1"] = x.values
        self.y["individual1"] = y.values
        self.ci["individual1"] = ci.values

        print(f"✅ Coordinates stored: {x.shape[0]} frames × {x.shape[1]} bodyparts")
    ...
    def retrain(self):
        """retrain current model with updated behavioural labels if needed"""
        
        if not hasattr(self, "model") or self.model is None:
            show_info("No model is loaded")
            return
        
        if not hasattr(self, "zebdata"):
            show_info("No training data is loaded")
            return
        
        
        
        #try to get updated training data
        try: 
            train_data = self.zebdata.data
            train_labels = self.zebdata.labels
        except AttributeError:
            show_info("zebdata missing data or labels")
            return
        
        if len(train_data) == 0 or len(train_labels) == 0:
            show_info("no training lables found.")
        
        print(f"training on {len(train_data)} samples with shape {train_data.shape}. ")
        
        # these dependencies are probably already loaded.
        ...
        try: 
            show_info("retraining model")
            trainer.fit(self.model, train_dataloaders=loader)
            show_info("Retraining complete")
        except Exception as e:
            show_info(f"Error during retraining: {e}")
    ...
    def on_model_change(self, model_name: str):
         ...
         show_info("loading model: {model_name}")
         
         ModelClass = {
             "ST-GCN-3BLOCK": ST_GCN_18,
             "ST-GCN_LSTM": ST_GCN_LSTM,
             "ST_GCN": ST_GCN_18_3BLOCK   
         }.get(model_name)
         
         self.model = ModelClass(
             in_channel=self.numChannels, 
             num_class=self.numlabels,
         ...
         show_info(f"Loaded Model: {model_type}")
         
      
      # if you need to reload weights 
      # self.model.load_state_dict(torch.load(self.current_checkpoint_path))
      
      
    def start_analysis(self):
        """where you trigger automatic_process_video() and model runs. """
